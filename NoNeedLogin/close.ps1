...
$procs = Get-CimInstance Win32_Process -Filter "Name = 'pythonw.exe'" |
    Where-Object { $_.CommandLine -like '*D:\files\using\Web\hymns\NoNeedLogin\app.py*' }

if (-not $procs) {
    Write-Host "No matching process found (D:\files\using\Web\hymns\NoNeedLogin\app.py)." -ForegroundColor Yellow
} else {
    foreach ($p in $procs) {
